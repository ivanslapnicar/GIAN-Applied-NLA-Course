...
# ╔═╡ 54e6e593-a87c-40cf-ac4f-56086bda1ff7
using PlutoUI, LinearAlgebra, Images, VideoIO, JLD

# ╔═╡ 7c919bd5-e6d1-44c5-b745-6fcd8385a93c
md"""
# Sparse + Low-Rank Splitting of Video

We have learned that the background image (scene) usually has a low rank.

Moving objects are sparse.

> Therefore, sparse + low-rank splitting could extract moving objects from a surveillance video.
"""

# ╔═╡ 98e1f6a7-c38d-4e3a-b613-b26d677e330f
md"""
We need functions from the notebook
[Sparse + Low-Rank Splitting](L13+Sparse+++Low-Rank+Splitting.jl).
"""

# ╔═╡ dcfceff8-69c6-4872-af82-38614b9cfad9
# Shrinkage
function Shr(x::Array{T},τ::T) where T
    sign.(x).*max.(abs.(x).-τ,zero(T))
end

# ╔═╡ 148702d8-8c9f-4f50-b8d5-70c6faf86961
...
# ╔═╡ 85f35847-620d-469c-a131-25f72a402d85
...
# ╔═╡ 7395b9f1-d6f2-45e7-82a4-d3fc62314d30
# For compilation
A₀=rand(3,3)

# ╔═╡ d5ee0050-2e42-4359-8f8f-b8aa4db7c20a
L₀,S₀,iter₀=PCPAD(A₀)

# ╔═╡ 86f7ce0e-6a07-40fc-b4d0-dc2b8e5be1f6
rank(L₀), L₀+S₀-A₀

# ╔═╡ 893fd586-86c1-4969-b54b-da0b3fffd4a8
L₀

# ╔═╡ a7160d72-8244-45f2-890b-74922f766f59
varinfo(VideoIO)

# ╔═╡ bed433cb-bb22-43ef-ad7f-7734f76aac04
frames=load("files/visor.avi")

# ╔═╡ 84c9459b-e100-4040-8386-59cc96e3166b
frames[1]

# ╔═╡ 6aab4ecb-1c4a-46cd-8e90-cfdddcc6ba07
typeof(frames[1])

# ╔═╡ a4a30f3a-4317-4447-9184-eac75cd813fd
VideoIO.get_duration("files/visor.avi")

# ╔═╡ 2ee1f80a-1912-46e6-a939-1b18b8640502
# Number of frames
length(frames)

# ╔═╡ f6df48b4-6395-4c5c-869e-3601e394363c
# See particular frame
frames[1401]

# ╔═╡ d9a88e38-093e-4d03-9eb5-91dff95e02a3
# Make shorter video clip
# clip=frames[1401:1450]
clip=map.(Gray,frames[1401:1450])

# ╔═╡ ba382355-fb15-41ba-ae0d-bf61327f759e
size(clip[1])

# ╔═╡ e3b7e1df-e0ce-478f-8496-f067a0af8e1e
begin
...
# ╔═╡ a3b346f7-5b94-4514-9416-d04f88011bcd
size(A)

# ╔═╡ 8c235587-d944-4574-af0d-3d49a0510a9a
norm(A)
...
begin
	...
	s₁=S[:,1]
	s₂=reshape(s₁,mᵢ,nᵢ)
	map(Gray{Normed{UInt8,8}},clamp01.(s₂.+0.5))
end

# ╔═╡ e5fdfe56-a69e-46e7-b546-37cb125c2c3a
begin
	LowRank=similar(clip)
	Sparse=similar(clip)
	for i=1:n
	    LowRank[i]=reshape(L[:,i],mᵢ,nᵢ)
	    Sparse[i]=clamp01.(reshape(S[:,i],mᵢ,nᵢ).+0.5)
	end
end
...
# ╔═╡ d659f564-974f-411c-92c7-53bc450bf5bd
@save "files/visor_results.jld" LowRank, Sparse

# ╔═╡ b532975f-7d2f-4f0d-9392-9054eeb0d0d9
# @load "files/visor_results.jld"

# ╔═╡ 1f39fe82-0753-4afb-9142-7b55f46a8989
LowRank[1]

# ╔═╡ fb7a42c5-6e3b-4f76-ab97-e5c5c6786d7f
Sparse[1]

# ╔═╡ 662abe99-97e0-4d16-96f9-c00f11fd386a
# Save the LowRank part a bit slower (framerate=10) for playing
VideoIO.save("files/LowRank.mp4",LowRank,framerate=10)

# ╔═╡ 204aa22c-6981-45e6-adb3-54e8f6442275
LocalResource("files/LowRank.mp4")

# ╔═╡ 6c9783db-15cf-449d-8fe8-5b354b02e79c
# Save the Sparse part for playing
VideoIO.save("files/Sparse.mp4",Sparse,framerate=10)

# ╔═╡ 5b473632-50f6-49e8-bf2f-4b185edcfdc6
LocalResource("files/Sparse.mp4")
...
manifest_format = "2.0"
project_hash = "752be6d20415c1357f32736b2dc4b9c8bc776186"
